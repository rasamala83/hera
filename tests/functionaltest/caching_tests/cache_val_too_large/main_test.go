...
func before() error {
	...
	err := testutil.DBDirect("drop table if exists testValTooLargeForJuno", os.Getenv("MYSQL_IP"), "heratestdb", testutil.MySQL)
	if err != nil {
		return err
	}
	err = testutil.DBDirect(
		"create table testValTooLargeForJuno (id INT, val MEDIUMTEXT)", os.Getenv("MYSQL_IP"), "heratestdb",
		testutil.MySQL,
	)
	if err != nil {
		return err
...
func TestTTLCacheValTooLargeForJuno(t *testing.T) {
	...
	testutil.RunDML("INSERT into hera_sql_caching (query_id, sqlhash, sqltext, bind_variables, TTL_sec, enable_shadow_test, tableName, invalidation_clause, caching_enabled, cache_by_corrid, caching_enabled_apps, remarks, hera_module) VALUES  ('1', '3520330359', 'SelectQueryWithLargeResultSet', 'abc=123', 30, 'N', 'MyTestTable', '', 'Y', 'Y', 'all','', 'hera-test')")
