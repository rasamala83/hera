...
func before() error {
	...
	if strings.HasPrefix(os.Getenv("TWO_TASK"), "tcp") {
		...
		err = testutil.DBDirect(
			"create table jdbc_hera_cache_test ( ID BIGINT, INT_VAL BIGINT, STR_VAL VARCHAR(500))",
			os.Getenv("MYSQL_IP"), "heratestdb", testutil.MySQL,
		)
		if err != nil {
			return err
...
func TestTTLCacheJunoUnreachableInitiallyDuringStartup(t *testing.T) {
	logger.GetLogger().Log(logger.Debug, "TestTTLCacheJunoUnreachableInitiallyDuringStartup begin +++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++\n")
	...
	if testutil.RegexCountFile("coordinator DispatchCachingSession for GET returned:.*connection refused", "hera.log") < 1 {
	...
	if testutil.RegexCountFile("Error in setRecordToCache:.*connection refused", "hera.log") < 1 {
		t.Fatalf("Error: should have exited from setRecordToCache")
	}
	...
	if testutil.RegexCountFile("coordinator DispatchCachingSession for GET returned:.*connection refused", "hera.log") > 1 {
		t.Fatalf("Error: should not have failed with connection refused")
	}
	...
	logger.GetLogger().Log(logger.Debug, "TestTTLCacheJunoUnreachableInitiallyDuringStartup done +++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++\n")
}
