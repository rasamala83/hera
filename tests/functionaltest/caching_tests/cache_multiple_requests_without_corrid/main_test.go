...
func before() error {
	...
	if strings.HasPrefix(os.Getenv("TWO_TASK"), "tcp") {
		...
		err = testutil.DBDirect(
			"INSERT into hera_sql_caching (query_id, sqlhash, sqltext, bind_variables, TTL_sec, enable_shadow_test, tableName, invalidation_clause, caching_enabled, remarks, hera_module) VALUES  ('1', '2733177372', 'select id, int_val from jdbc_hera_cache_txn_test where id=?', 'id=1', 30, 'N', 'jdbc_hera_cache_taf_test', '', 'Y', '', 'hera-test')",
			os.Getenv("MYSQL_IP"), "heratestdb", testutil.MySQL,
		)
		if err != nil {
			return err
...
// GET (Cache MISS) + SET
func TestTTLCacheWithDifferentCorrIdAndCacheByCorrIdDisabled(t *testing.T) {
	logger.GetLogger().Log(logger.Debug, "TestTTLCacheWithDifferentCorrIdAndCacheByCorrIdDisabled begin +++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++\n")
	...
	for index := 1; index < 10; index++ {
		...
		go func() {
			...
			mux.SetCalCorrID(generateRandomCorrId())
			defer conn.Close()
			if err != nil {
	...
	logger.GetLogger().Log(logger.Debug, "TestTTLCacheWithDifferentCorrIdAndCacheByCorrIdDisabled done +++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++\n")
}
