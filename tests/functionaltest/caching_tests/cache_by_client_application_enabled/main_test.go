...
// GET (Cache MISS) + SET
func TestTTLCacheByClientApplicationEnabled(t *testing.T) {
	logger.GetLogger().Log(logger.Debug, "TestTTLCacheByClientApplicationEnabled begin +++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++\n")
	...
	mux := gosqldriver.InnerConn(conn)
	mux.SetClientInfo("clientApplicationC", testutil.GetHostname())
	...
	if testutil.RegexCountFile(".*GET\t2904134799\t0.*", "cal.log") < 1 {
		t.Fatalf("Error: request should be served from cache")
	}
	...
	if testutil.RegexCountFile(".*SET\t2904134799\t0.*", "cal.log") > 1 {
		t.Fatalf("Error: should not see SET... It should be served from cache")
	}
	...
	if testutil.RegexCountFile(".*GET\t2904134799\t0.*", "cal.log") < 2 {
	...
	logger.GetLogger().Log(logger.Debug, "TestTTLCacheByClientApplicationEnabled done +++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++\n")
}
