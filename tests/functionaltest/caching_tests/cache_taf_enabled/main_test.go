...
func before() error {
	...
	if strings.HasPrefix(os.Getenv("TWO_TASK"), "tcp") {
		...
		err = testutil.DBDirect(
			"INSERT into hera_sql_caching (query_id, sqlhash, sqltext, bind_variables, TTL_sec, enable_shadow_test, tableName, invalidation_clause, caching_enabled, remarks, hera_module) VALUES  ('1', '2166545700', 'select id, int_val from jdbc_hera_cache_taf_test where id=?', 'id=1', 300, 'N', 'jdbc_hera_cache_taf_test', '', 'Y', '', 'hera-test')",
			os.Getenv("MYSQL_IP"), "heratestdb", testutil.MySQL,
		)
		if err != nil {
			return err
