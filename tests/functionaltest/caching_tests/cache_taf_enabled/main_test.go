...
func before() error {
	...
	if strings.HasPrefix(os.Getenv("TWO_TASK"), "tcp") {
		...
		err = testutil.DBDirect("create table jdbc_hera_cache_taf_test ( ID BIGINT, INT_VAL BIGINT, STR_VAL VARCHAR(500))", os.Getenv("MYSQL_IP"), "heratestdb", testutil.MySQL)
		if err != nil {
			return err
		}

		err = testutil.DBDirect(
			"INSERT into hera_sql_caching (query_id, sqlhash, sqltext, bind_variables, TTL_sec, enable_shadow_test, tableName, invalidation_clause, caching_enabled, cache_by_corrid, caching_enabled_apps, remarks, hera_module) VALUES  ('1', '2166545700', 'select id, int_val from jdbc_hera_cache_taf_test where id=?', 'id=1', 300, 'N', 'jdbc_hera_cache_taf_test', '', 'Y', 'N', 'all', '', 'hera-test')",
			os.Getenv("MYSQL_IP"), "heratestdb", testutil.MySQL,
		)
		if err != nil {
			return err
		...
		err = testutil.DBDirect(
			"/*cmd*/insert into "+tableName+"(id, int_val, str_val) VALUES(1,"+fmt.Sprint(time.Now().Unix())+", \"val 1\")",
			os.Getenv("MYSQL_IP"), "heratestdb", testutil.MySQL,
		)
		if err != nil {
			return err
