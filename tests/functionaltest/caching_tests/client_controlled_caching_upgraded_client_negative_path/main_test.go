...
func populateCache() error {
	...
	// Should be sent only when the server is upgraded...Triggers CmdServerUnexpectedCommand
...
// GET (Cache MISS) + SET
func TestClientControlledCacheUpgradedClientNegativePath(t *testing.T) {
	logger.GetLogger().Log(logger.Debug, "TestClientControlledCacheUpgradedClientNegativePath begin +++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++\n")
	...
	if err != nil {
		logger.GetLogger().Log(logger.Debug, "Error populateCache():", err.Error())
	}
	...
	if testutil.RegexCountFile(".*ERROR\tCmdServerUnexpectedCommand.*", "cal.log") < 3 {
		t.Fatalf("Error: should see unexpected command events for cmd: 30, 31, 32")
	}

	if testutil.RegexCountFile(".*worker writing to mux.*4:1006", "hera.log") < 3 {
		t.Fatalf("Error: should see unexpected command events for cmd: 30, 31, 32")
	}

	logger.GetLogger().Log(logger.Debug, "TestClientControlledCacheUpgradedClientNegativePath done +++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++\n")
}
