...
func before() error {
	...
	if strings.HasPrefix(os.Getenv("TWO_TASK"), "tcp") {
		...
		testutil.DBDirect("create table jdbc_hera_cache_txn_test ( ID BIGINT, INT_VAL BIGINT, STR_VAL MEDIUMTEXT)", os.Getenv("MYSQL_IP"), "heratestdb", testutil.MySQL)

		testutil.DBDirect("INSERT into hera_sql_caching (query_id, sqlhash, sqltext, bind_variables, TTL_sec, enable_shadow_test, tableName, invalidation_clause, caching_enabled, remarks, hera_module) VALUES  ('1', '2243240848', 'select id, int_val, str_val from jdbc_hera_cache_txn_test where id=?', 'id=1', 5, 'N', 'jdbc_hera_cache_taf_test', '', 'Y', '', 'hera-test')", os.Getenv("MYSQL_IP"), "heratestdb", testutil.MySQL)
	}
	return nil
}
...
func TestTTLCacheNoCorrIdSQLHashWithDifferentDataSize(t *testing.T) {
	logger.GetLogger().Log(logger.Debug, "TestTTLCacheSelectQueriesInParallelUniqueKeyError begin +++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++\n")
	...
	conn.Close()
	cancel()
	time.Sleep(6 * time.Second) //Wait for TTL 5 seconds expaires
	...
	if testutil.RegexCountFile("2243240848 CachingEnabled for  SET : true", "hera.log") < 1 {
	...
	logger.GetLogger().Log(logger.Debug, "TestTTLCacheSelectQueriesInParallelUniqueKeyError done +++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++\n")
}
