...
func cfg() (map[string]string, map[string]string, testutil.WorkerType) {
	...
	appcfg["cache_endpoint"] = "10.176.9.146:5080" //Point to incorrect endpoint to simulate connection refused errors 
...
func TestTTLCacheJunoCertsMissing(t *testing.T) {
	...
	if testutil.RegexCountFile("coordinator DispatchCachingSession for GET returned:.*tls: bad certificate", "hera.log") < 1 {
	...
	if testutil.RegexCountFile("coordinator DispatchCachingSession for SET returned:.*tls: bad certificate", "hera.log") < 1 {
