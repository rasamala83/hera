...
func TestTTLCacheWithDifferentCorrIdAndCacheByCorrIdDisabled(t *testing.T) {
	logger.GetLogger().Log(logger.Debug, "ttlCacheWithDifferentCorrIdAndCacheByCorrIdDisabled begin +++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++\n")
	...
	if testutil.RegexCountFile("coordinator DispatchCachingSession for SET returned: error: unique key violation", "hera.log") < 1 {
		t.Fatalf("Error:should see error: unique key violation while submitting SQL hash in parallel even with different corrIds")
	}

	logger.GetLogger().Log(logger.Debug, "ttlCacheWithDifferentCorrIdAndCacheByCorrIdDisabled done +++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++\n")
}
