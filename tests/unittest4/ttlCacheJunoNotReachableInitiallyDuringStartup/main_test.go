...
func TestTTLCacheJunoUnreachableInitiallyDuringStartup(t *testing.T) {
	...
	sb, err = testutil.StartSpeedBumpProxy(9090, destAddr, 10, 100, 2)
	...
	if testutil.RegexCountFile("T.*SET\t658232417\t0", "cal.log") < 1 {
		t.Fatalf("Error: SET should have success")
	}
	...
	if testutil.RegexCountFile("T.*GET\t658232417\t2.*connect: connection refused", "cal.log") < 1 {
		t.Fatalf("Error: One Get should failed connection refused")
	}
